        ...
        for (auto &end_point : u_neighbors) {
          ...
          for (int w = 0; w < max_channels; ++w) {
            if (fabs(cplex.getValue(x_mn_uv[m][n][u][v]) - 1) < EPS
              ...
              printf("Virtual edge (%d, %d) --> physical edge (%d, %d): Channel %d\n", m, n,
                     u, v, w);
              if (outfile) {
                fprintf(outfile,
        ...
        for (auto &end_point : u_neighbors) {
          ...
          for (int w = 0; w < max_channels; ++w)  {
            if (fabs(cplex.getValue(x_mn_uv[m + offset][n + offset][u][v]) - 1) <
                EPS && fabs(cplex.getValue(l_mn_uv_w[m + offset][n + offset][u][v][w]) - 1) < EPS)  {
              printf(
                  "Shadow virtual edge of (%d, %d) --> physical edge (%d, %d): Channel %d\n",
                  m, n, u, v, w);
             if (outfile) {
